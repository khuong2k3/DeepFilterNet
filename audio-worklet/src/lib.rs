...
#[no_mangle]
pub unsafe fn df_new(model_bytes: &[u8], atten_lim: f32) -> *mut WasmAudioProcessor {
    let channels = 1;
    //let model_bytes = unsafe {
    //    std::slice::from_raw_parts(model_ptr, model_size)
    //};
    ...
    Box::into_raw(Box::new(WasmAudioProcessor::new(m)))
}
